...
apply_name_corrections <- function(names_vector, corrections = name_corrections) {
  result <- names_vector
...
standardize_early_structure <- function(df, fiscal_year) {
  ...
  dept_col <- base::intersect(base::names(df), c("department", "home_department"))[1]
...
standardize_late_structure <- function(df, fiscal_year) {
  ...
  regular_col <- base::names(df)[stringr::str_detect(base::names(df), "regular_earnings|regular$")][1]
  overtime_col <- base::names(df)[stringr::str_detect(base::names(df), "overtime_earnings|overtime$")][1]
  additional_col <- base::names(df)[stringr::str_detect(base::names(df), "^fy\\d+_additional$|additional_earnings")][1]
...
stage_1_import <- function(registry = file_registry, data_path = paths$data_raw) {
  ...
  base::message("\n  Combined ", base::nrow(combined), " rows from ", 
                base::length(all_data), " files")
...
stage_2_clean <- function(df) {
  ...
  base::message("  Unique standardized names: ", dplyr::n_distinct(result$name_std))
...
stage_3_corrections_and_keys <- function(df, corrections = name_corrections) {
  ...
  base::message("  Created ", dplyr::n_distinct(result$employee_key), " unique employee keys")
...
stage_5_finalize <- function(df) {
  ...
  base::message("  Fiscal years: ", base::paste(base::sort(base::unique(result$fiscal_year)), collapse = ", "))
...
run_pipeline_with_intermediates <- function(registry = file_registry, 
