...
profile_text_columns <- function(df) {
  ...
  purrr::map_dfr(text_cols, function(col) {
    ...
    dplyr::tibble(
      ...
      has_unicode_hyphen = base::any(
        stringr::str_detect(x, "[\u2010-\u2015\u2212]"),
        na.rm = TRUE
      ),
      has_leading_trailing_space = base::any(
        x != stringr::str_squish(x),
        na.rm = TRUE
      ),
      min_nchar = base::min(base::nchar(x), na.rm = TRUE),
      max_nchar = base::max(base::nchar(x), na.rm = TRUE),
      sample_values = base::paste(
        utils::head(base::unique(stats::na.omit(x)), 5),
        collapse = " | "
      )
    )
  })
...
profile_excel_file <- function(filename, data_path = paths$data_raw) {
  ...
  if (base::nrow(invisible_char_issues) > 0) {
    base::message(
      "WARNING: Found invisible character issues in ",
      dplyr::n_distinct(invisible_char_issues$column),
      " column(s)"
    )
  }
...
print_profile_summary <- function(profile) {
  ...
  if (base::nrow(issues) > 0) {
    ...
    for (i in base::seq_len(base::nrow(issues))) {
      ...
      flags <- c()
      if (row$has_nbsp) {
        flags <- c(flags, "non-breaking spaces")
      }
      if (row$has_unicode_hyphen) {
        flags <- c(flags, "unicode hyphens")
      }
      if (row$has_leading_trailing_space) {
        flags <- c(flags, "leading/trailing spaces")
      }
      base::cat(
        "  ",
        row$column,
        ": ",
        base::paste(flags, collapse = ", "),
        "\n"
      )
    }
    base::cat("\n")
  ...
  # Similar names
  if (
    !base::is.null(profile$similar_names) &&
      base::nrow(profile$similar_names) > 0
  ) {
...
compare_structure <- function(
  ...
  base::cat(
    ...
    best_match,
    " (",
    base::round(base::max(similarities) * 100),
    "% similar)\n\n"
  )
  ...
  if (base::max(similarities) > 0.8) {
    suggested_type <- registry$structure_type[
      registry$fiscal_year == best_match
    ]
