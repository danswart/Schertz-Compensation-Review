...
checkpoint_a_source_reconciliation <- function(combined_df, 
...
checkpoint_b_name_variations <- function(df, 
  ...
  base::message("Scanning ", base::length(names_to_check), " unique standardized names...")
    ...
    similar <- similar |>
      dplyr::mutate(
        possibly_corrected = purrr::map2_lgl(name1, name2, function(n1, n2) {
          base::any(purrr::map_lgl(corrections$pattern, ~ {
  ...
  if (n_similar == 0) {
    base::message("No similar name pairs found (edit distance 1-", max_distance, ").")
  } else {
    n_uncorrected <- base::sum(!similar$possibly_corrected)
    base::message("Found ", n_similar, " similar name pair(s).")
    if (n_uncorrected > 0) {
      base::message("  ", n_uncorrected, " may need review (not covered by existing corrections).")
    }
  }
  
  # This checkpoint is informational - always "PASS" but with warnings
  has_issues <- (n_case_vars > 0) | (n_similar > 0 & base::any(!similar$possibly_corrected))
  ...
  base::list(
    ...
    summary = dplyr::tibble(
      check = c("Case variations", "Similar names (edit dist 1-2)", "Possibly needing review"),
      count = c(
        n_case_vars,
        n_similar,
...
checkpoint_c_key_validation <- function(df) {
  ...
  if (n_dups == 0) {
    ...
    base::message("WARNING: Found ", n_dups, " duplicate key instances within fiscal years!")
  }
...
check_key_entities <- function(df, entities = key_entities) {
  ...
  purrr::pmap_dfr(
    ...
    function(name_pattern, job_title_pattern, expected_years, note) {
      ...
      matches <- df |>
        dplyr::filter(
          stringr::str_detect(name, stringr::regex(name_pattern, ignore_case = TRUE)),
          stringr::str_detect(job_title, stringr::regex(job_title_pattern, ignore_case = TRUE))
        ) |>
...
compare_external_totals <- function(df, 
  ...
  calculated <- df |>
    ...
    dplyr::summarise(calculated_total = base::sum(amount, na.rm = TRUE), .groups = "drop")
...
checkpoint_d_completeness <- function(df,
  ...
  if (base::nrow(entity_check) > 0) {
    ...
    if (all_entities_found) {
      base::message("All ", base::nrow(entity_check), " key entities found in expected years.")
    } else {
      base::message("WARNING: ", n_missing, " key entity/year combination(s) missing!")
      missing <- entity_check |> dplyr::filter(!found)
      for (i in base::seq_len(base::nrow(missing))) {
        base::message("  - ", missing$name_pattern[i], " (", missing$job_title_pattern[i], 
                      ") missing from: ", missing$missing_years[i])
      }
    }
  } else {
  ...
  if (base::nrow(external_check) > 0) {
    all_within_tolerance <- base::all(external_check$within_tolerance, na.rm = TRUE)
    ...
    if (all_within_tolerance) {
      ...
      for (i in base::seq_len(base::nrow(outside))) {
        base::message("  - ", outside$fiscal_year[i], ": ", 
                      scales::percent(outside$pct_diff[i], accuracy = 0.1), " difference")
      }
    }
  } else {
...
run_reconciliation <- function(combined_df,
                                cleaned_df,
                                final_df,
                                registry = file_registry,
                                corrections = name_corrections,
  ...
  checkpoint_b <- checkpoint_b_name_variations(cleaned_df, corrections = corrections)
  ...
  base::message("\n")
  base::message(base::strrep("=", 70))
  base::message("  OVERALL STATUS: ", base::ifelse(overall_pass, "PASS", "FAIL"))
...
generate_reconciliation_report <- function(reconciliation_results,
  ...
  filename <- base::paste0("reconciliation", test_suffix, "_", date_str, ".html")
...
<p><strong>Generated:</strong> ', base::format(results$timestamp, "%Y-%m-%d %H:%M:%S"), '</p>

<div class="summary-box">
  <h2>Overall Status: ', overall_status, '</h2>
  <ul>
    <li>Checkpoint A (Source Reconciliation): ', status_badge(results$checkpoint_a$pass), '</li>
    <li>Checkpoint B (Name Variations): ', status_badge(results$checkpoint_b$pass), 
        base::ifelse(results$checkpoint_b$has_warnings, ' <span class="warn">(warnings)</span>', ''), '</li>
    <li>Checkpoint C (Key Validation): ', status_badge(results$checkpoint_c$pass), '</li>
    <li>Checkpoint D (Completeness): ', status_badge(results$checkpoint_d$pass), '</li>
  </ul>
</div>
...
', df_to_html_table(
  results$checkpoint_a$details |>
    ...
    dplyr::select(fiscal_year, filename, source_rows, imported_rows, rows_match,
                  source_total, imported_total, totals_match)
), '
...
')
  ...
  if (base::nrow(results$checkpoint_b$similar_names) > 0) {
    ...
    html <- base::paste0(html, '
<h3>Similar Name Pairs (showing first ', n_to_show, ')</h3>
', df_to_html_table(
...
')
  ...
  if (base::nrow(results$checkpoint_d$entity_check) > 0) {
    html <- base::paste0(html, df_to_html_table(results$checkpoint_d$entity_check))
  } else {
    html <- base::paste0(html, '<p><em>No key entities defined in configuration.</em></p>')
  }
...
')
  ...
  if (base::nrow(results$checkpoint_d$external_comparison) > 0) {
    html <- base::paste0(html, df_to_html_table(
    ...
    html <- base::paste0(html, '<p><em>No external totals defined for comparison.</em></p>')
  }
