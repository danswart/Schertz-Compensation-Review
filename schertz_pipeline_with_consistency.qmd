...
rank_changes |>
  ...
  dplyr::select(department, rank_FY20, rank_FY24, rank_direction,
                total_compensation_FY20, total_compensation_FY24) |>
  flextable::flextable() |>
  flextable::set_header_labels(
    rank_FY20 = "FY20 Rank",
    rank_FY24 = "FY24 Rank",
    rank_direction = "Change",
...
**What changed:**
- Removed `rank_change` from the `dplyr::select()` 
- Changed conditional formatting to use `stringr::str_starts(rank_direction, "↑")` and `stringr::str_starts(rank_direction, "↓")` instead of checking `rank_change > 0`

Now the formatting logic uses the visible column content, so we don't need the hidden helper column.
