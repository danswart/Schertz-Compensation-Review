...
```{r}
#| label: plot-city-manager-total-cagr
#| fig-width: 8
#| fig-height: 13
...
# Calculate CAGR per person
cagr_by_person <- cm_summary |>
  dplyr::group_by(name) |>
  dplyr::arrange(fiscal_year) |>
  dplyr::summarise(
    cagr = ((dplyr::last(total_compensation) / dplyr::first(total_compensation))^(1/(dplyr::n()-1)) - 1) * 100,
    .groups = "drop"
  )

# Build caption string
cagr_text <- cagr_by_person |>
  dplyr::mutate(label = paste0(name, ": ", round(cagr, 1), "%")) |>
  dplyr::pull(label) |>
  paste(collapse = " | ")
...
cm_summary |>
  ...
  ggplot2::geom_point(size = 3) +
  ggplot2::geom_text(
    ggplot2::aes(label = scales::dollar(total_compensation)),
    vjust = -1.0,
    size = 5,
    show.legend = FALSE,
    color = "black"
  ...
  ggplot2::labs(
    ...
    y = "Total Compensation",
    color = "Employee",
    caption = paste("Compound Annual Growth Rate:", cagr_text)
  )
...
```{r}
#| label: plot-total-growth-cagr
#| fig-width: 8
#| fig-height: 13

# Extract data first
city_totals <- reconciliation$checkpoint_d$our_totals

# Calculate CAGR
city_cagr <- city_totals |>
  dplyr::arrange(fiscal_year) |>
  dplyr::summarise(
    cagr = ((dplyr::last(total_compensation) / dplyr::first(total_compensation))^(1/(dplyr::n()-1)) - 1) * 100
  ) |>
...
city_totals |>
  ...
  ggplot2::labs(
    ...
    x = "Fiscal Year",
    y = "Total Compensation",
    caption = paste0("Compound Annual Growth Rate: ", round(city_cagr, 1), "%")
  )
```
