...
paths <- base::list(
  ...
  data_clean = here::here("data-clean"),
  reports = here::here("reports"),
  output_test = here::here("output-test")
)
...
base::cat("Defined mappings for", base::length(file_mappings), "fiscal years\n")
```
------------------------------------------------------------------------
...
extract_component_totals <- function(profile, mapping) {
  ...
  # Extract source total_compensation IF it exists
  tc_pattern <- mapping$total_columns$total_compensation
  if (!base::is.null(tc_pattern)) {
    ...
    if (!base::is.null(tc_col)) {
      ...
      # Column pattern defined but not found - use total_earnings as fallback
      totals$source_total_compensation <- totals$source_total_earnings
    }
  } else {
    # For FY20-22: no total_compensation column defined, use total_earnings as the check
    totals$source_total_compensation <- totals$source_total_earnings
  }
